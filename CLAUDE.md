...
| Test code shown to viewers | **none — see rule 28** |
...
**When code exceeds the 125-line budget, the surplus becomes an exercise** (see rule 13). Do not solve everything on camera. The viewer learning by implementing is the point, not a compromise.
...
20. **Project ownership is fixed.** `MiniCode.Cli` owns terminal I/O and nothing else; `MiniCode.Agent` owns agent construction, the loop, context and planning; `MiniCode.Tools` owns tool definitions and result shaping; `MiniCode.Workspace` owns path boundaries and file access; `MiniCode.Infrastructure` owns shell, Git, config and telemetry. Dependencies run one way — `Cli → Agent → Tools → { Workspace, Infrastructure }` — and the two leaves reference nothing internal. Put a capability where it permanently belongs the first time; a later Module having to move it is churn the viewer sits through.
21. **Verified API facts — do not re-derive these, and do not trust older samples.**
    - `AgentSession` has **no** messages property. History is reached via `AgentSessionExtensions.TryGetInMemoryChatHistory(out List<ChatMessage>)` / `SetInMemoryChatHistory(...)`.
    - Sessions come from `agent.CreateSessionAsync()`; `AgentThread`/`GetNewThread()` is superseded.
    - Non-streaming runs return `AgentResponse` — `AgentRunResponse` no longer exists. Streaming yields `AgentResponseUpdate`.
    - `ChatClientAgentOptions` has **no** `Instructions` property; instructions live on `ChatOptions.Instructions` and are re-sent every request, so compaction cannot drop an `AGENTS.md`. The simple `ChatClientAgent(chatClient, instructions, name, description, tools, …)` constructor is leaner and is what MiniCode uses.
...
22. The course is delivered in five Phases. Each of the first four ends with a Phase Capstone: a full runnable snapshot of the solution, plus a lesson that exercises the Phase's work end to end and states honestly what the tool still cannot do. Capstones introduce no concept absent from syllabus.md.
...
29. Before declaring a Module done:
